...
import { Loader2, Gift, Award, AlertCircle, Copy, Sparkles, Clock, CheckCircle, BadgeCheck, Tag, Coins, Info, MessageCircle, CreditCard, AlertTriangle } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
export default function TeacherRewardsPage() {
  ...
  async function fetchData() {
    try {
      ...
      showErrorToast(
        "خطأ في تحميل البيانات",
        error.message || "حدث خطأ أثناء تحميل المكافآت. يرجى المحاولة مرة أخرى."
      )
    } finally {
      setIsLoading(false)
    }
  ...
  const confirmRedemption = async () => {
    ...
    try {
      ...
      showErrorToast(
        "خطأ في استبدال المكافأة",
        "حدث خطأ أثناء استبدال المكافأة. يرجى المحاولة مرة أخرى."
      )
    } finally {
    ...
    <div className="container mx-auto p-6 max-w-7xl">
      ...
      {selectedReward && (
        <ConfirmationDialog
          ...
          description="هل أنت متأكد من رغبتك في استبدال هذه المكافأة؟"
        ...
        >
          <div className="flex flex-col items-center justify-center gap-4 py-4">
            ...
            <div className="text-center">
              <div className="text-lg font-bold">{selectedReward.name}</div>
              <div className="text-sm text-muted-foreground">{selectedReward.description}</div>
              <div className="flex items-center justify-center mt-2 gap-1 text-amber-600 bg-amber-50 rounded-full px-3 py-1 text-sm font-medium">
                <CreditCard className="h-4 w-4 mr-1" />
                {selectedReward.points_cost} نقطة
              </div>
            </div>
            {userPoints < selectedReward.points_cost && (
              <Alert variant="destructive" className="mt-2">
                <AlertTriangle className="h-4 w-4" />
                <AlertTitle>رصيد غير كافي</AlertTitle>
                <AlertDescription>
                  رصيدك الحالي ({userPoints} نقطة) غير كافي لاستبدال هذه المكافأة
                </AlertDescription>
              </Alert>
            )}
          </div>
