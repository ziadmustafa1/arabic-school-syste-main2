  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const notifications = selectedStudents.map((studentId) => ({
        user_id: studentId,
        title: "إضافة نقاط",
        content: `تم إضافة ${pointsToAdd} نقطة ${formData.description ? `(${formData.description})` : ""}`,
      }))
  ...
  return (
    <DashboardLayout>
      <div className="container mx-auto py-6">
        <h1 className="text-2xl font-bold">إضافة نقاط إيجابية</h1>
        <p className="text-muted-foreground mb-6">إضافة نقاط إيجابية للطلاب بناءً على إنجازاتهم وسلوكهم</p>
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="space-y-2">
                  <Label htmlFor="description">سبب منح النقاط (اختياري)</Label>
                  <Textarea
                    id="description"
                    name="description"
                ...
                <Button type="submit" className="w-full" disabled={isSubmitting || selectedStudents.length === 0}>
                  {isSubmitting ? (
                    ...
                    <>
                      ...
                      إضافة النقاط ({selectedStudents.length} طالب)
                    </>
                  )}
                </Button>
              </form>
